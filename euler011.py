...
linhas = list(conjunto.split("\n"))
for linha in linhas:
	numeros = list(linha.split(" "))
	matriz.append(numeros)
	for numero in numeros:
		a = 1

def maior_produto_linha(matriz):
	maior = 0
	for linha in matriz:
		for x in range(0, 17):
			produto = int(linha[x]) * int(linha[x+1]) * int(linha[x+2]) + int(linha[x+3])
			if produto > maior:
				maior = produto
	return maior
...
def maior_produto_diagonal_esq_dir(matriz):
	maior = 0
	for x in range(0,17):
		for y in range(0,17):
